...
<div class="uk-article ka-content">
	...
	<?php if ($this->params->get('show_feed_link', 1)):
		$link = 'index.php?option=com_kinoarhiv&view=movies&format=feed&Itemid=' . $this->itemid; ?>
		<div class="feed-link">
	...
	<?php if (count($this->items) > 0):
		if (is_object($this->filtersData) && $this->filtersData->exists('movies')):
						...
						<?php if (!$this->user->guest && $this->params->get('link_favorite') == 1): ?>
							<p class="favorite">
								<?php if ($item->favorite == 1): ?>
									<a href="<?php echo JRoute::_('index.php?option=com_kinoarhiv&view=movies&task=movies.favorite&action=delete&Itemid=' . $this->itemid . '&id=' . $item->id); ?>" class="cmd-favorite delete" data-ka-msg-place="div"><?php echo JText::_('COM_KA_REMOVEFROM_FAVORITE'); ?></a>
								<?php else: ?>
									<a href="<?php echo JRoute::_('index.php?option=com_kinoarhiv&view=movies&task=movies.favorite&action=add&Itemid=' . $this->itemid . '&id=' . $item->id); ?>" class="cmd-favorite add" data-ka-msg-place="div"><?php echo JText::_('COM_KA_ADDTO_FAVORITE'); ?></a>
								<?php endif; ?>
							</p>
						<?php endif; ?>
					</div>
	...
	<?php endif;
		...
		<br/>
		<div><?php echo ($this->params->get('search_movies_enable') == 1 && $this->filtersData->exists('movies')) ? JText::sprintf('COM_KA_SEARCH_ADV_N_RESULTS', 0) : KAComponentHelper::showMsg(JText::_('COM_KA_NO_ITEMS')); ?></div>
	<?php endif; ?>
</div>
