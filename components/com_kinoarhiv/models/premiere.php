 ...
 * @since  3.0
 */
class KinoarhivModelPremiere extends JModelItem
{
	/**
	...
	{
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('premiere.id');
			...
			try
			{
				...
				$query = $db->getQuery(true)
					->select(
						$this->getState(
							'item.select',
							'p.id, p.premiere_date, p.country_id, p.info, cn.name, cn.code'
						)
					)
					...
					->where($db->quoteName('p.movie_id') . ' = ' . $pk)
					->where($db->quoteName('p.language') . ' IN (' . $db->quote(JFactory::getLanguage()->getTag()) . ',' . $db->quote('*') . ')')
