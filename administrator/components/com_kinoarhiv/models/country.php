...
class KinoarhivModelCountry extends JModelForm
{
	...
	/**
	 * Removes country or countries from database.
	 *
	 * @return  boolean  True on success.
	 *
	 * @since   3.0
	...
	public function save($data)
	{
		...
		try
		{
			...
			if (empty($data['id']))
			{
				$sessionData = $app->getUserState('com_kinoarhiv.countries.' . $user->id . '.edit_data');
				$sessionData['id'] = $db->insertid();
				$app->setUserState('com_kinoarhiv.countries.' . $user->id . '.edit_data', $sessionData);
			}
		}
		catch (Exception $e)
		{
