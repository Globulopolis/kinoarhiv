			...
			for ($i = 0, $n = $totalReviews; $i < $n; $i++):
				...
				<div class="review-row">
					...
					<div class="review-title corner-top">
						...
						<span><a href="<?php echo JRoute::_('index.php?option=com_kinoarhiv&view=album&id=' . $this->item->id . '&limitstart=' . $this->pagination->limitstart . '&Itemid=' . $this->itemid) . '#review-' . $review->id; ?>"
					...
					<?php if (!$this->user->guest && ($allowReviewForm == 1 && $itemAllowReviewsForm == 1)): ?>
						...
						<div class="review-footer corner-bottom">
							...
							<?php if ($this->user->get('isRoot') || ($this->user->authorise('core.delete.reviews', 'com_kinoarhiv') && $review->uid == $this->user->get('id'))): ?>
								<a href="<?php echo JRoute::_('index.php?option=com_kinoarhiv&task=reviews.delete&review_id=' . $review->id . '&id=' . $review->item_id . '&' . JSession::getFormToken() . '=1&return=' . base64_encode('view=album&id=' . $review->item_id)); ?>"
