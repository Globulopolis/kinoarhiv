	...
	<div class="ui-widget">
		<div>
			<?php if ($item_trailer->embed_code != ''):
				...
				<div id="video">
					<img src="components/com_kinoarhiv/assets/themes/component/<?php echo $this->params->get('ka_theme'); ?>/images/icons/loading.gif">
					Loading the player...
				</div>
				<script type="text/javascript">
					jwplayer('video').setup({
						playlist: [{
							file: '<?php echo $item_trailer->files['video'][0]['src']; ?>',
							image: '<?php echo $item_trailer->screenshot; ?>',
							tracks: [
								<?php if (count($item_trailer->files['subtitles']) > 0):
									foreach ($item_trailer->files['subtitles'] as $subtitle): ?>
								{
									file: '<?php echo $subtitle['file']; ?>',
								...
								{
									file: '<?php echo $item_trailer->files['chapters']['file']; ?>',
									kind: 'chapters'
								}
								<?php endif; ?>
							]
